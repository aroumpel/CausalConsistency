fci <- function (suffStat, indepTest, alpha, labels, p, skel.method = c("stable", "original", "stable.fast"),
                 type = c("normal", "anytime", "adaptive"), fixedGaps = NULL, fixedEdges = NULL, NAdelete = TRUE, 
                  m.max = Inf, pdsep.max = Inf, rules = rep(TRUE, 10), doPdsep = TRUE, biCC = FALSE, 
                  conservative = FALSE, maj.rule = FALSE, verbose = FALSE, aggressivelyPreventCycles = FALSE) 
{
  cl <- match.call()
  if (!missing(p)) 
  ...
  if (doPdsep) {
    ...
    if (conservative || maj.rule) {
      ...
      sk. <- pc.cons.intern(tmp.pdsep, suffStat, indepTest, 
                            alpha, verbose = verbose, version.unf = c(1, 1), maj.rule = maj.rule)
    ...
    if (conservative || maj.rule) {
      ...
      nopdsep <- pc.cons.intern(skel, suffStat, indepTest, 
                                alpha, verbose = verbose, version.unf = c(2, 1), maj.rule = maj.rule)
  ...
  if (verbose) 
    cat("\nDirect egdes:\n=============\nUsing rules:", which(rules), 
        "\nCompute collider:\n")
  res <- udag2pag(pag = 1*G, sepset, rules = rules, unfVect = tripleList, verbose = verbose, aggressivelyPreventCycles=aggressivelyPreventCycles)
